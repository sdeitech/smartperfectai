	...
	<h2>Contact Form Advanced Database</h2>
	 <div id="poststuff">
		...
		<table class="wp-list-table widefat fixed media paginated" id="property-lead-table" data-page-length='10'>
			...
			<tbody>
				<?php 
				$get_all_posts = get_post_meta($cf7Selector,'cf7-adb-data');
				global $wpdb;
  				$table = "wp_postmeta";
  				//$result_posts = $wpdb->get_results("SELECT * FROM $table WHERE post_id = $cf7Selector Order by meta_id DESC ");
  				$result = get_post_meta($cf7Selector,'cf7-adb-data'); 
